...
import { StarOutlined, ForkOutlined,PullRequestOutlined } from '@ant-design/icons';
...
const RepositoryCard: React.FC<RepositoryCardProps> = ({ name, description, stars, forks,prs, onClick }) => {
  return (
    <Card onClick={onClick} hoverable style={{ marginBottom: '16px',flex: '0 0 calc(50% - 8px)', backgroundColor: 'var(--card-color)', transition: 'background-color 0.3s ease'}}>
      ...
      <Space style={{ marginTop: '8px' }}>
        ...
        <Text style={{color: 'var(--text-color)', transition: 'color 0.3s ease'}}><PullRequestOutlined /> {prs}</Text>
      </Space>
    </Card>
  );
};
